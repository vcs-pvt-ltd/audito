...
export function AuditPdfRenderer({ report, entityTree }: AuditPdfRendererProps) {
...
let cursorY = boxY + boxHeight + 35;
      ...
      const auditDays =
        report.audit.start_date && report.audit.end_date
          ? Math.ceil(
              (new Date(report.audit.end_date).getTime() -
                new Date(report.audit.start_date).getTime()) /
                (1000 * 60 * 60 * 24)
            )
          : 0;
  ...
  { label: "Duration",      value: `${auditDays} days`,                                                      color: [244, 63, 94] },
];
