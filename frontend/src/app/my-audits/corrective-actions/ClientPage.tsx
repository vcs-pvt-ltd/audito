                      ...
                      <div className="p-4 space-y-4">
                        ...
                        {it.remarks && (
                          <div className="p-3 rounded-xl bg-white/[0.03] border border-white/10">
                            <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1">Auditor Remarks</p>
...
export default function CorrectiveActionsPage() {
  ...
  const handleCreateCap = async () => {
    ...
    const manualTitle = capTitle.trim();
    if (!manualTitle) {
      setError("Please enter a CAP title.");
      return;
    }
    setCreatingCap(true);
    setError("");
      ...
      <main className="container mx-auto px-4 sm:px-6 py-4 sm:py-6 md:py-8 pt-16 sm:pt-6 md:pt-8">
        ...
        {loading ? (
          ...
          <div className="space-y-4 max-w-5xl mx-auto">
            ...
            {/* Existing CAP summary - Responsive */}
            {existingCap && (
              <div className="bg-white/5 backdrop-blur-sm rounded-lg sm:rounded-xl p-4 sm:p-5 border border-secondary-500/20">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs sm:text-sm font-semibold text-white">CAP Created</p>
                    </div>
                  </div>
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                    <div className="flex items-center gap-2">
                      <span className={`text-[9px] sm:text-xs px-2 py-0.5 sm:py-1 rounded-full border font-medium capitalize ${CAP_STATUS_BADGE[existingCap.status] || "bg-gray-500/15 text-gray-400 border-gray-500/30"}`}>
            ...
            <div className="bg-white/5 backdrop-blur-sm rounded-lg sm:rounded-xl p-4 sm:p-5 space-y-3 border border-white/10">
              ...
              {!saved && items.length > 0 && (
                <div className="flex items-start sm:items-center gap-2 px-2.5 sm:px-3 py-2 rounded-lg bg-amber-500/10 border border-amber-500/20">
                  ...
                  <p className="text-[10px] sm:text-xs text-amber-300">Fill in due dates, then save to enable CAP Plan creation.</p>
                </div>
              )}
              ...
              {items.length === 0 ? (
                ...
                <p className="text-xs sm:text-sm text-gray-600 italic">Entity tree is not available for this audit.</p>
              )}
            </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2 pb-6 sm:pb-8">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto order-1 sm:order-2">
                  ...
                  {saved && !existingCap && (
                    <button
                      onClick={() => {
                        setCapTitle(audit?.title ? `CAP: ${audit.title}` : "");
                        setShowCreateCapModal(true);
                      }}
                    ...
                    >
                      {creatingCap ? <div className="w-3 h-3 sm:w-4 sm:h-4 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <PlusCircle size={12} />}
                      Create CAP
                    </button>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-white/15 glass shadow-2xl">
            <div className="px-5 py-4 border-b border-white/10">
              <h3 className="text-base font-semibold text-white">Create CAP</h3>
              <p className="text-xs text-gray-400 mt-1">Add a CAP title before creating.</p>
            </div>
            <div className="px-5 py-4 space-y-2">
              <label className="text-xs text-gray-400">CAP Title</label>
              <input
                value={capTitle}
