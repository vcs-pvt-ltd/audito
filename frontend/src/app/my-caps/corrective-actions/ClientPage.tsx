      ...
      <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-3">
        {evidence.map((file, index) => {
          ...
          return (
            ...
            >
              {kind === "image" ? (
                <img src={url} alt={file.file_name || "CAP evidence"} className="h-20 w-full rounded-md object-cover" />
              ) : (
              ...
              {entityItems.map((it, idx) => {
                ...
                return (
                  <CorrectiveActionAccordion
                    key={it.response_id}
                    name="cap-corrective-action"
                    questionNumber={idx + 1}
                    questionText={it.question_text}
                    accent="orange"
                    headerBadges={<span className="rounded border border-orange-500/10 bg-orange-500/5 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-widest text-orange-500/50">Sub-CAP Required</span>}
                  >
                    ...
                    <div className="space-y-3 p-3 sm:p-4">
                      ...
                      {it.remarks && (
                        <div className="rounded-lg border border-white/[0.06] bg-white/[0.03] px-3 py-2">
                          <p className="mb-1 text-[10px] font-bold uppercase tracking-wider text-gray-600">Remarks</p>
...
export default function CapCorrectiveActionsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (subCapsRes.success && subCapsRes.data) {
        const subCapsData = subCapsRes.data as { caps?: (SubCapSummary & { parent_cap_id?: string | null })[] };
        const subs = (subCapsData.caps || []).filter(
          (subCap) => String(subCap.parent_cap_id ?? "") === String(capId)
        );
  ...
  const handleCreateSubCap = async () => {
    ...
    if (!subCapTitle.trim()) { toast("Please enter a Sub-CAP title.", "error"); return; }
