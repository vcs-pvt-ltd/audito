...
interface Cap {
  id: number; cap_plan_code: string; audit_id?: number;
  title: string; status: string; start_date?: string; end_date?: string;
}
interface CapQuestion {
  id: number; entity_code: string; org_tree_id?: number | null;
  question_text: string; total_marks: string | number; status: string;
  order_index?: number; ca_description?: string;
}
interface CapResponse {
  id?: number; cap_question_id: number; response_text: string | null;
  selected_option_ids?: string | null | number[]; marks_obtained?: number;
...
interface CapProgress {
  entity_code: string; entity_name?: string; org_tree_id?: number | null;
...
interface TreeNode {
  ...
  edge_id?: number | null; children: TreeNode[];
}

// ─── Helpers ──────────────────────────────────────────────────────

function sameEntityInstance(
  entityCode: string, orgTreeId: number | null | undefined,
  item: { entity_code: string; org_tree_id?: number | null },
  { allowGeneric = false }: { allowGeneric?: boolean } = {}
) {
  const n = orgTreeId ?? null;
  const i = item.org_tree_id ?? null;
  if (allowGeneric && i === null) return item.entity_code === entityCode;
  if (i !== null || n !== null) return item.entity_code === entityCode && i === n;
...
function EntityTreeSection({
  node, questionsByEntity, responses, progMap, depth = 0,
}: {
  node: TreeNode; questionsByEntity: Record<string, CapQuestion[]>;
  responses: Record<number, CapResponse>; progMap: Map<string, CapProgress>; depth?: number;
}) {
      ...
      <div className="rounded-xl border border-white/[0.08] overflow-hidden">
        ...
        >
          ...
          <div className="flex items-center gap-3 shrink-0">
            {hasChildren && <span className="text-[10px] text-gray-600 hidden sm:inline">{node.children.length} sub</span>}
        ...
        {expanded && (
          <div className="border-t border-white/[0.06]">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 space-y-1.5">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          {isDone ? (
                            <span className="shrink-0 inline-flex items-center gap-1 text-[10px] text-emerald-400">
                              <CheckCircle2 size={10} /> Done
                            </span>
                          ) : (
                            <span className="shrink-0 inline-flex items-center gap-1 text-[10px] text-amber-400">
                        ...
                        {q.ca_description && (
                          <p className="text-[10px] text-amber-400/70">
                            <span className="font-medium text-amber-500/60 mr-1">Requirement:</span>{q.ca_description}
                          </p>
                        )}
                        <div className="flex flex-wrap items-center gap-2 text-[10px]">
                          <span className={`font-semibold ${obtained >= totalMarks && totalMarks > 0 ? "text-emerald-400" : obtained > 0 ? "text-secondary-400" : "text-gray-500"}`}>
                            {obtained}/{totalMarks} pts
                          </span>
                          {resp?.remarks && (
              ...
              <div className={`px-3 pb-3 space-y-0 ${hasQuestions ? "border-t border-white/[0.06] pt-2" : "pt-2"}`}>
                {node.children.map((child) => (
                  <EntityTreeSection
                    ...
                    node={child} questionsByEntity={questionsByEntity}
                    responses={responses} progMap={progMap} depth={depth + 1}
                  />
                ))}
              </div>
            )}
            ...
            {!hasQuestions && !hasChildren && (
              <p className="px-4 py-3 text-xs text-gray-600 italic">No CAP actions required for this entity.</p>
            )}
          </div>
        )}
      </div>
...
export default function MyCapReportPage() {
  ...
  const load = useCallback(async () => {
    ...
    const detail = detailRes.data as {
      ...
      questions: CapQuestion[]; progress: CapProgress[]; tree?: TreeNode | null;
    };
    setCap(detail.cap);
  ...
  const byEntity = useMemo(() => {
    ...
    const walk = (node: TreeNode) => {
      const edgeId = (node as any).edge_id ?? null;
      const nodeKey = `${node.code}__${edgeId ?? "null"}`;
      const nodeQs = questions.filter(q => sameEntityInstance(node.code, edgeId, q, { allowGeneric: true }));
      if (nodeQs.length > 0) m[nodeKey] = nodeQs;
  ...
  const prunedTree = useMemo(() => {
    ...
    const prune = (n: TreeNode | null): TreeNode | null => {
      if (!n) return null;
      const children = (n.children || []).map(prune).filter(Boolean) as TreeNode[];
      if (hasQ(n) || children.length > 0) return { ...n, children };
      return null;
    };
    return prune(tree);
  }, [tree, questions]);
  ...
  const progMap = useMemo(() => {
    if (!tree) return new Map<string, CapProgress>();
    const map = new Map<string, CapProgress>();
    const agg = (node: TreeNode): CapProgress => {
      const edgeId = (node as any).edge_id ?? null;
      const nodeKey = `${node.code}__${edgeId ?? "null"}`;
      const nodeQs = questions.filter(q => sameEntityInstance(node.code, edgeId, q, { allowGeneric: true }));
      let total_questions = nodeQs.length, answered_questions = 0, total_marks = 0, obtained_marks = 0;
      nodeQs.forEach(q => {
      ...
      (node.children || []).forEach(child => {
        ...
        total_marks += (c.total_marks || 0);
        obtained_marks += (c.obtained_marks || 0);
      });
      const result: CapProgress = {
        entity_code: node.code, entity_name: node.name, org_tree_id: edgeId,
        total_questions, answered_questions, total_marks, obtained_marks,
  ...
  const capPdfReport = useMemo(() => {
    ...
    const normalizedQuestions = questions.map(q => ({ id: q.id, question_text: q.question_text, answer_type: "free_text", total_marks: Number(q.total_marks || 0), entity_code: q.entity_code, order_index: q.order_index || 0, options: [] }));
    const normalizedResponses = questions.map(q => {
      const r = responseMap[q.id];
      return { id: r?.id || q.id, question_id: q.id, entity_code: q.entity_code, org_tree_id: q.org_tree_id ?? null, answer_text: r?.response_text || null, selected_option_ids: (r?.selected_option_ids as string | null) || null, marks_obtained: Number(r?.marks_obtained || 0), remarks: r?.remarks || null, cap_required: Number(r?.cap_required || 0), status: r?.status || q.status || "plan", evidence: r?.evidence || [] };
    });
    const toKey = (code: string, treeId?: number | null) => `${code}__${treeId ?? "null"}`;
    const marksByEntity: Record<string, { total: number; obtained: number }> = {};
    for (const q of normalizedQuestions) {
      const k = toKey(q.entity_code, questions.find(x => x.id === q.id)?.org_tree_id ?? null);
    ...
    for (const r of normalizedResponses) {
      const k = toKey(r.entity_code, r.org_tree_id ?? null);
      if (!marksByEntity[k]) marksByEntity[k] = { total: 0, obtained: 0 };
      marksByEntity[k].obtained += Number(r.marks_obtained || 0);
    }
    const normalizedProgress = progress.map(p => ({ entity_code: p.entity_code, entity_name: p.entity_name || entitiesMap.get(p.entity_code)?.entity_name || p.entity_code, total_questions: Number(p.total_questions || 0), answered_questions: Number(p.answered_questions || 0), total_marks: Number(p.total_marks || 0) || marksByEntity[toKey(p.entity_code, p.org_tree_id ?? null)]?.total || 0, obtained_marks: Number(p.obtained_marks || 0) || marksByEntity[toKey(p.entity_code, p.org_tree_id ?? null)]?.obtained || 0, status: p.status || "plan", org_tree_id: p.org_tree_id ?? null }));
    ...
    return {
      audit: { id: cap.id, audit_code: cap.cap_plan_code, title: cap.title, status: cap.status, start_date: String(auditMeta?.start_date || cap.start_date || ""), end_date: String(auditMeta?.end_date || cap.end_date || ""), organization_name: auditMeta?.organization_name || "", organization_email: auditMeta?.organization_email || "", organization_phone: auditMeta?.organization_phone || "", auditor_name: auditMeta?.auditor_name || [auditMeta?.assigned_auditor_first_name, auditMeta?.assigned_auditor_last_name].filter(Boolean).join(" ") || "", auditor_email: auditMeta?.auditor_email || "", auditor_phone: auditMeta?.auditor_phone || "", entities: Array.from(entitiesMap.values()) },
      responses: normalizedResponses, progress: normalizedProgress, questions: normalizedQuestions,
      summary: { total_marks: totalMarks, obtained_marks: obtainedMarks, score_pct: totalMarks > 0 ? Math.round((obtainedMarks / totalMarks) * 100) : 0, total_questions: normalizedQuestions.length, answered_questions: answeredQuestions, total_entities: entitiesMap.size },
    };
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {cap && <p className="text-sm text-gray-400 mt-0.5 hidden sm:block truncate">{cap.title}</p>}
              </div>
            </div>
            {cap && (
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                ...
                <div className="glass rounded-xl border border-white/[0.08] p-4 sm:p-5 flex flex-col items-center text-center">
                  <div className="relative inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 mb-2">
                    <svg className="w-16 h-16 sm:w-20 sm:h-20 -rotate-90" viewBox="0 0 36 36">
                      ...
                      <path d="M18 2.0845a15.9155 15.9155 0 0 1 0 31.831a15.9155 15.9155 0 0 1 0-31.831"
                        fill="none" stroke={getScoreStroke(pct)} strokeWidth="3"
                        strokeLinecap="round" strokeDasharray={`${pct}, 100`} />
                    </svg>
                    <span className={`absolute text-base sm:text-lg font-bold ${pct === 100 ? "text-emerald-400" : pct >= 50 ? "text-secondary-400" : "text-amber-400"}`}>
                      {pct}%
                    </span>
                  </div>
                  <p className="text-xs font-medium text-white">Completion</p>
                  <p className="text-[10px] text-gray-500 mt-0.5">{answeredQ}/{totalQ} actions done</p>
                </div>
                ...
                <div className="glass rounded-xl border border-white/[0.08] p-4 sm:p-5">
                  ...
                  <p className="text-2xl font-bold text-white">{answeredQ}</p>
                  <p className="text-xs text-gray-400 mt-0.5">of {totalQ} actions</p>
