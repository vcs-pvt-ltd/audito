...
const ANSWER_TYPE_META = {
  ...
  single_option: { label: "Single Option", icon: ToggleLeft, color: "text-green-400", desc: "Select one option (e.g. Yes / No)" },
  multiple_options: { label: "Multiple Options", icon: CheckSquare, color: "text-purple-400", desc: "Select multiple options (checkboxes)" },
  dropdown: { label: "Drop Down", icon: List, color: "text-orange-400", desc: "Select one from a dropdown list" },
};
...
/** Flatten all nodes in a tree into a flat list */
        ...
        return (
          <div key={i} className="flex items-center gap-2 sm:gap-4">
            <div className="flex flex-col items-center gap-1.5 min-w-[60px] sm:min-w-[80px]">
              <div
                className={`w-9 h-9 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center transition-all duration-300 ${isActive
              ...
              <span
                className={`text-[10px] sm:text-[11px] font-bold uppercase tracking-wider transition-colors duration-300 ${isActive ? "text-secondary-400" : isCompleted ? "text-emerald-400/80" : "text-gray-600"
                  }`}
              >
                {s.name}
              </span>
  ...
  return (
    <div className="glass rounded-xl border border-white/10 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-white/[0.02] border-b border-white/[0.06]">
        <span className="w-6 h-6 rounded-full bg-secondary-500/15 border border-secondary-500/20 flex items-center justify-center text-secondary-400 text-[11px] font-bold shrink-0">
          {index + 1}
        </span>
        ...
        <span className="text-gray-600 text-xs ml-1">{meta.desc}</span>
        <button
          onClick={onDelete}
          className="ml-auto p-1 rounded text-gray-600 hover:text-red-400 transition-colors"
          ...
          <div>
            ...
            >
              {Object.entries(ANSWER_TYPE_META).map(([val, m]) => (
                <option key={val} value={val} className="bg-[#0c2218] text-white">{m.label}</option>
              ))}
            </select>
          </div>
        </div>
        ...
        {showOptions && (
          <div>
            <div className="flex items-center justify-between mb-2">
              <FieldLabel label="Options & Marks" required />
              <div className={`text-xs font-medium ${marksOk ? "text-green-400" : "text-amber-400"}`}>
                Total: {totalMarks.toFixed(1)} / 10
                {marksOk && <Check size={12} className="inline ml-1" />}
              </div>
            </div>
            <div className="space-y-2">
              {question.options.map((opt, oi) => (
                <div key={oi} className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-500 text-[10px] font-bold shrink-0">
                    {oi + 1}
                  </div>
                  <input
                    value={opt.text}
                    ...
                    className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-secondary-500/50 focus:ring-1 focus:ring-secondary-500/20 transition-all"
                  />
                  <div className="flex items-center gap-1 shrink-0">
                    <input
                      ...
                      placeholder="0"
                      className="w-16 bg-white/5 border border-white/10 rounded-lg px-2 py-2 text-sm text-white text-center focus:outline-none focus:border-secondary-500/50 focus:ring-1 focus:ring-secondary-500/20 transition-all"
                    />
            ...
            {question.options.length < 5 && (
              <button
                onClick={addOption}
                className="mt-2 flex items-center gap-1.5 text-xs text-secondary-400 hover:text-secondary-300 transition-colors"
              >
            ...
            {!marksOk && question.options.some(o => o.marks) && (
              <p className="mt-1.5 text-xs text-amber-400 flex items-center gap-1">
                <AlertCircle size={11} />
                Marks must add up to exactly 10.
              </p>
            )}
          </div>
        )}
...
function TreeEntityNode({
  ...
  const typeColor =
    ENTITY_TYPE_COLORS[node.entity_type] ?? "bg-gray-500/20 text-gray-300 border-gray-500/30";
  ...
  return (
    <div>
      ...
      <button
        ...
        className="w-full flex items-center gap-2 py-2.5 px-3 rounded-lg hover:bg-white/[0.04] transition-colors text-left group"
      >
        {expanded
      ...
      {/* Expanded content — indented with left border tree line */}
      {expanded && (
      ...
      <div className="glass rounded-2xl w-full max-w-sm shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          <p className="text-sm text-gray-400">
            Select the entity from your org tree to add questions for:
          </p>
          <select
            value={selected}
          ...
          >
            ...
            {entities.map(n => (
              <option key={n.code} value={n.code} className="bg-[#0c2218] text-white">
                [{n.entity_type}] {n.name}
              </option>
            ))}
          </select>
          <div className="flex gap-3 pt-1">
            <button onClick={onClose} className="flex-1 py-2.5 rounded-lg text-sm text-gray-400 hover:text-white border border-white/10 hover:border-white/20 transition-all">
              Cancel
            </button>
            <button
              disabled={!selected}
              onClick={() => {
                if (node) { onAdd(node.code, node.entity_type, node.name); onClose(); }
              }}
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-2xl rounded-2xl border border-white/10 shadow-2xl flex flex-col max-h-[85vh]">
        ...
        {/* Tree-node list (hierarchy) */}
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
          {prunedRoot ? (
            ENTITY_TYPE_COLORS[prunedRoot.entity_type]
              ? <ExcelTreeEntityNode node={prunedRoot} questions={questions} />
              : (prunedRoot.children ?? []).map((child) => (
                <ExcelTreeEntityNode key={child.code} node={child} questions={questions} />
              ))
          ) : (
            <div className="rounded-xl border border-white/10 bg-white/[0.02] p-10 text-center">
              <p className="text-white font-medium mb-1">No hierarchy available</p>
              <p className="text-gray-500 text-sm">Upload succeeded, but org tree is not available to preview hierarchy.</p>
            </div>
          )}
        </div>
        ...
        <div className="px-6 py-4 border-t border-white/[0.08] flex items-center gap-3 justify-between shrink-0">
          ...
          >
            <Check size={14} />
            Add {questions.length} Question{questions.length !== 1 ? "s" : ""} to Checklist
          </button>
        </div>
      </div>
    </div>
...
function pruneTreeByEntityCodes(node: TreeNode, entityCodes: Set<string>): TreeNode | null {
  ...
  if (
    entityCodes.has(nodeKey(node.code, node.edge_id ?? null)) ||
    entityCodes.has(node.code) ||
...
function ExcelTreeEntityNode({ node, questions }: { node: TreeNode; questions: QuestionForm[] }) {
  ...
  return (
    <div>
      <button
        onClick={() => setExpanded(p => !p)}
        className="w-full flex items-center gap-2 py-2 px-2.5 rounded-lg hover:bg-white/[0.04] transition-colors text-left"
      >
        ...
        <span className="text-sm font-medium text-white flex-1 truncate">{node.name}</span>
        {totalQs > 0 && (
          <span className="text-[11px] text-secondary-400 bg-secondary-500/10 border border-secondary-500/20 px-1.5 py-0.5 rounded-full shrink-0">{totalQs}Q</span>
        )}
      </button>
          ...
          {ownQs.length > 0 && (
            <div className="py-1.5 space-y-2">
              {ownQs.map((q, i) => {
                ...
                const Icon = meta.icon;
                return (
                  <div key={q.id} className="flex items-start gap-2.5 py-2 px-2.5 rounded-lg bg-white/[0.015] border border-white/[0.05] hover:bg-white/[0.03] transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-gray-200 leading-snug">{q.question_text}</p>
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        <span className={`inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded bg-white/5 border border-white/10 ${meta.color}`}>
...
export default function CreateChecklistPage() {
  ...
  // Step state
  const [step, setStep] = useState(1); // 1 = Basic, 2 = Settings, 3 = Questions
  ...
  const [currencies, setCurrencies] = useState<string[]>(["$", "€", "£", "¥", "₹", "AED", "SAR"]); // Initial common list

  // Fetch all world currencies from a free API
  useEffect(() => {
    fetch("https://api.exchangerate-api.com/v4/latest/USD")
      ...
      .then(data => {
        if (data && data.rates) {
          ...
          // Merge with common symbols and set
          setCurrencies(prev => {
            const symbols = ["$", "€", "£", "¥", "₹"];
    ...
    const load = async () => {
      ...
      if (treeRes.success && treeRes.data) {
        ...
        if (d.tree) {
          ...
          setTreeEntities(flat);
          setTreeRoot(d.tree);
        }
      }

      // If editing, load existing checklist
      if (isEdit && editId) {
        ...
        if (clRes.success && clRes.data) {
          ...
          // Build a flat lookup from the tree for entity names
          ...
          const entityGroups = (cl.entity_questions as Array<{
            ...
            questions: Array<{
              id: number; question_text: string; answer_type: string;
          ...
          for (const eg of entityGroups) {
            ...
            const entityDisplayName =
              flatForLookup.find(n => nodeKey(n.code, n.edge_id ?? null) === nodeKey(eg.entity_code, egTreeId))?.name || eg.entity_code;
  ...
  const entityGroups = questions.reduce<Record<string, { code: string; orgTreeId: number | null; type: string; name: string; qs: QuestionForm[] }>>((acc, q) => {
    const k = questionKey(q);
    if (!acc[k]) {
      acc[k] = { code: q.entity_code, orgTreeId: q.org_tree_id ?? null, type: q.entity_type, name: q.entity_name, qs: [] };
    }
    acc[k].qs.push(q);
    return acc;
  }, {});
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit && checklistId) {
        ...
        const newQuestions = questions.filter(q => q.id.startsWith("local-"));
        if (newQuestions.length > 0) {
          const qPayloads: QuestionPayload[] = newQuestions.map(q => ({
  ...
  const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!res.success) {
        const detailErrors = Array.isArray((res as { errors?: unknown }).errors)
          ? ((res as { errors: string[] }).errors)
          : [];
      ...
      // Ensure org tree is available BEFORE building uploaded questions.
      // Otherwise Excel items would have org_tree_id=null and the hierarchy preview
      // would incorrectly show as unavailable.
      ...
      for (const it of payload?.items || []) {
        ...
        loadedQs.push({
          ...
          org_tree_id: it.org_tree_id ?? null,
          entity_type: it.entity_type,
          entity_name: entityDisplayName,
          question_text: it.question_text,
          answer_type: it.answer_type,
          options: (it.options || []).map(o => ({ text: o.option_text, marks: String(o.marks) })),
        });
      }
  ...
  return (
    <div className="min-h-full bg-transparent flex flex-col relative w-full">
      <main className="flex-1 p-4 sm:p-6 lg:p-8 pt-20 lg:pt-8">
        ...
        {/* Error banner */}
        {saveError && (
          <div className="mb-8 flex items-start gap-4 p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm animate-in fade-in slide-in-from-top-2">
            <AlertCircle size={18} className="mt-0.5 shrink-0" />
            <div className="flex-1 font-medium">{saveError}</div>
            <button onClick={() => setSaveError("")} className="p-1 rounded-lg hover:bg-white/5 transition-colors"><X size={16} /></button>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="glass rounded-2xl p-4 sm:p-6 md:p-8 border border-white/10 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
            <SectionHeader
              num={1}
              label="Checklist Foundation"
              desc="Set the core identity of your checklist. Choose a clear name that helps auditors identify the task."
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="lg:col-span-2">
                ...
                <input
                  ...
                  placeholder="e.g. Factory Floor Safety Audit Q1"
                  className={inputCls}
                />
              </div>
              <div className="lg:col-span-2">
              ...
              <div className="lg:col-span-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {mediaFileName && (
                    <span className="text-sm text-gray-300 truncate max-w-xs">{mediaFileName}</span>
                  )}
                  {formData.media_path && !uploadingMedia && (
                    <button
                      type="button"
                      onClick={() => { setFormData(p => ({ ...p, media_path: "" })); setMediaFileName(""); }}
                ...
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      ...
                      if (res.success && res.data) {
                        const d = res.data as { file_path: string };
                        setFormData(p => ({ ...p, media_path: d.file_path }));
                        setMediaFileName(file.name);
                      } else {
                        setSaveError(res.message || "Failed to upload media.");
                      }
                ...
                <p className="mt-1.5 text-xs text-gray-600 flex items-center gap-1">
                  <Info size={11} /> Upload an image, PDF, or video as reference material.
                </p>
              </div>
            </div>
          </div>
        ...
        {step === 2 && (
          <div className="glass rounded-2xl p-4 sm:p-6 md:p-8 border border-white/10 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
            <SectionHeader
              num={2}
              label="Configuration & Schedule"
              desc="Define how this checklist behaves. Set its type, repetition schedule, and required resources."
              onboarding={searchParams.get("onboarding") === "1"}
            />
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-6 sm:gap-y-8">
              ...
              <div className="flex flex-col">
                ...
                <div className="mt-auto flex gap-2">
                  <select
                    ...
                    onChange={e => setForm("checklist_type_id", e.target.value)}
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-lg bg-secondary-500/10 text-secondary-400 border border-secondary-500/20 hover:bg-secondary-500/20 transition-all flex items-center justify-center whitespace-nowrap"
              ...
              <div className="flex flex-col">
                ...
                <div className="mt-auto flex items-center bg-white/5 border border-white/10 rounded-lg focus-within:border-secondary-500/50 focus-within:ring-1 focus-within:ring-secondary-500/20 transition-all overflow-hidden">
                  <input
                    type="number"
                    min="1"
                    value={formData.time_period_value}
                    onChange={e => setForm("time_period_value", e.target.value)}
                    placeholder="e.g. 3"
                    className="flex-1 w-full bg-transparent px-3.5 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none"
                  />
                  <div className="w-px h-6 bg-white/10 shrink-0"></div>
                  <select
                    value={formData.time_period_unit}
                    onChange={e => setForm("time_period_unit", e.target.value)}
                    className="bg-transparent pl-3 pr-8 py-2.5 text-sm text-gray-300 focus:outline-none cursor-pointer appearance-none outline-none border-none shrink-0"
                  ...
                  >
                    <option value="" disabled className="bg-transparent text-gray-500">Select unit *</option>
                    {DURATION_UNITS.map(u => (
                      <option key={u} value={u} className="bg-transparent text-white">{u.charAt(0).toUpperCase() + u.slice(1)}</option>
                    ))}
                  </select>
              ...
              <div className="flex flex-col">
                ...
                <div className="mt-auto flex items-center bg-white/5 border border-white/10 rounded-lg focus-within:border-secondary-500/50 focus-within:ring-1 focus-within:ring-secondary-500/20 transition-all overflow-hidden">
                  <input
                    type="number"
                    min="1"
                    value={formData.repeat_duration_value}
                    onChange={e => setForm("repeat_duration_value", e.target.value)}
                    placeholder="e.g. 6"
                    className="flex-1 w-full bg-transparent px-3.5 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none"
                  />
                  <div className="w-px h-6 bg-white/10 shrink-0"></div>
                  <select
                    value={formData.repeat_duration_unit}
                    onChange={e => setForm("repeat_duration_unit", e.target.value)}
                    className="bg-transparent pl-3 pr-8 py-2.5 text-sm text-gray-300 focus:outline-none cursor-pointer appearance-none outline-none border-none shrink-0"
                    style={{ backgroundImage: "url(\"data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%239CA3AF%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E\")", backgroundRepeat: "no-repeat", backgroundPosition: "right 0.75rem top 50%", backgroundSize: "0.65rem auto" }}
                  >
                    <option value="" disabled className="bg-transparent text-gray-500">Select unit *</option>
              ...
              <div className="flex flex-col">
                ...
                <div className="mt-auto flex items-center bg-white/5 border border-white/10 rounded-lg focus-within:border-secondary-500/50 focus-within:ring-1 focus-within:ring-secondary-500/20 transition-all overflow-hidden">
                  ...
                  >
                    {currencies.map(c => (
                      <option key={c} value={c} className="bg-transparent text-white">{c}</option>
                    ))}
                  </select>
                </div>
              </div>
              ...
              <div className="flex flex-col">
                ...
                <div className="mt-auto">
                  <input
                    ...
                    onChange={e => setForm("num_workers", e.target.value)}
                    placeholder="e.g. 5"
                    className={inputCls}
                  />
                </div>
        ...
        {/* ─── STEP 3: Questions per Entity ──────────────────────────── */}
        {step === 3 && (
          <div className="glass rounded-2xl p-4 sm:p-6 md:p-8 border border-white/10 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
            <SectionHeader
              num={3}
              label="Question Assignments"
              desc="Connect specific questions to entities in your organization structure. You can add them manually or bulk import via Excel."
              onboarding={searchParams.get("onboarding") === "1"}
            />

            {/* Excel bulk upload */}
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-5 p-3.5 rounded-xl bg-white/[0.02] border border-white/[0.06]">
              <FileText size={15} className="text-gray-400 shrink-0" />
              <span className="text-sm text-gray-400 flex-1">Bulk add questions via Excel</span>
              <div className="flex items-center gap-2 flex-wrap">
                <a
                  ...
                  onClick={async (e) => {
                    ...
                    if (!accessToken) return;
                    try {
                      await checklistApi.downloadExcelTemplate(accessToken);
                    } catch (err: unknown) {
                      setSaveError(err instanceof Error ? err.message : "Failed to download template.");
                    }
                  }}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-secondary-400 border border-secondary-500/20 hover:bg-secondary-500/10 hover:border-secondary-500/40 transition-all"
                >
                  <Download size={12} />
                  Download Template
                </a>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-white bg-secondary-500/20 border border-secondary-500/20 hover:bg-secondary-500/30 transition-all disabled:opacity-60"
                >
                  {uploadingExcel ? (
            ...
            {/* Excel result */}
            {uploadResult && (
              <div className={`mb-4 p-3.5 rounded-xl text-sm border ${uploadResult.errors?.length > 0
                ? "bg-amber-500/10 border-amber-500/20 text-amber-400"
                : "bg-green-500/10 border-green-500/20 text-green-400"
                }`}>
                <p className="font-medium mb-1">
                  {uploadResult.created_count} question(s) imported
                  {uploadResult.errors?.length > 0 && `, ${uploadResult.errors.length} row(s) skipped`}.
                </p>
                {uploadResult.errors?.length > 0 && (
                  <ul className="text-xs space-y-0.5 mt-2 text-amber-300/80">
            ...
            {treeEntities.length === 0 ? (
              ...
              <div className="glass rounded-xl border border-white/[0.08] p-3 space-y-0.5">
                {treeRoot
                  ? (ENTITY_TYPE_COLORS[treeRoot.entity_type]
                    /* root is a known org entity — render it */
                    ? <TreeEntityNode
                      node={treeRoot}
                      questions={questions}
                      allEntities={treeEntities}
                      onAddQuestion={addQuestion}
                      onChangeQuestion={updateQuestion}
                      onDeleteQuestion={deleteQuestion}
                    />
                    /* root is admin wrapper — render its children directly */
              ...
              onClick={() => {
                if (step === 1) {
                  if (!formData.name.trim()) {
                    setSaveError("Checklist name is required to continue.");
                    return;
                  }
                  if (!formData.description.trim()) {
                    setSaveError("Description is required to continue.");
                    return;
                  }
                }
                if (step === 2) {
                  if (!formData.checklist_type_id) {
                    setSaveError("Checklist type is required to continue.");
                    return;
                  }
                  if (!formData.time_period_value) {
                    setSaveError("Time period is required to continue.");
                    return;
                  }
                  if (!formData.time_period_unit) {
                    setSaveError("Time period unit is required to continue.");
                    return;
                  }
                  if (!formData.repeat_duration_value) {
                    setSaveError("Repeat duration is required to continue.");
                    return;
                  }
                  if (!formData.repeat_duration_unit) {
                    setSaveError("Repeat duration unit is required to continue.");
                    return;
                  }
                }
                setStep(s => s + 1);
        ...
        <ExcelConfirmModal
          ...
          onConfirm={(qs) => {
            ...
            // Jump to questions step so user can review
            setStep(3);
          }}
          onDiscard={() => {
            setShowUploadConfirmModal(false);
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-transparent/80 backdrop-blur-sm">
            <div className="bg-primary-900 max-w-sm w-full rounded-2xl border border-white/10 overflow-hidden shadow-2xl flex flex-col animate-in fade-in zoom-in duration-200">
              ...
              <div className="px-5 py-4 border-t border-white/5 bg-white/[0.01] flex items-center gap-3 justify-end">
                <button
                  onClick={() => setShowTypeModal(false)}
                  className="px-4 py-2 rounded-lg text-sm text-gray-300 hover:text-white hover:bg-white/5 transition-colors font-medium"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-2 rounded-lg text-sm bg-secondary-500 hover:bg-secondary-600 text-primary-950 font-semibold transition-colors disabled:opacity-50 flex items-center gap-2"
                >
